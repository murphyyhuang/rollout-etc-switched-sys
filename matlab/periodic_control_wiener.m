...
if ~exist('video_name', 'var')
    video_name = ['../avi/periodic_ctrl_tau',  num2str(period_tau), ...
...
if  ~load_flag
    ...
    wienerIntg = eye(state_dim) + s * linearA ...
            ...
            + 1/720 * s^6 * linearA^6;
    wienerIntg = wienerIntg * noise_magnit;
    ...
    save(['periodic_ctrl_tau', num2str(period_tau), '_q', num2str(var_q), '.mat'], ...
        'mat_Q_qtau', 'mat_S_qtau', 'mat_R_qtau', 'Phi_qtau');
else
...
if flag_anima
    ...
    % Animation
    for k = 1:20:fix(length(obsv_time) / 4)
...
save(['./tmp/periodic_wiener_ctrl_simu_tau', num2str(period_tau), '_q', num2str(var_q), '.mat'], ...
