...
if ~exist('video_name', 'var')
    video_name = ['../avi/rollout_et_ctrl_tau', num2str(period_tau), '_param1.avi'];
end
...
for t_index = 1:size(obsv_time, 2)-1
    ...
    if ~rem(t_index - 1, schedule_period)
        ...
        for iSche = 1:nSches
            cost_values = [cost_values, xi_tmp' * PList{iSche} * xi_tmp];
        end
        
        [~, minSche] = min(cost_values);
...
save(['./tmp/rollout_ctrl_simu_tau', num2str(period_tau), '_q', num2str(var_q), '.mat'], ...
